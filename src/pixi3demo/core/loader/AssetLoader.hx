...
class AssetLoader extends Loader {
	...
	public function addAsset(id:String, path:String, ?usePixelRatio:Bool = true) {
		if (!exists(id)) {
			...
			else Console.info("'" + baseUrl + path + "' not availabble in AssetList. Make sure the asset is available or run the build to update AssetsList");
		}
	}
	...
	public function getJson(id:String):Dynamic {
		...
		else Console.error("JSON wth index '" + id + "' not found");
		return null;
	}
	...
	public function getTexture(id:String):Texture {
		...
		else Console.error("Texture with id '" + id + "' not found");
		return null;
	}
	...
	public function getTextureFromSpritesheet(id:String, frame:String):Texture {
		...
		if (resource != null && resource.isJson && resource.textures != null) {
			...
			else Console.error("Texture with frame'" + frame + "' not found in " + id);
		}
		else Console.error("Spritesheet with id '" + id + "' not found");
		return null;
	}
